ami = "ami-0f9de6e2d2f067fca" #22.04
instance_type = "t3a.2xlarge"
project_name = "mlops"
tags = {
    Environment = "dev"
    Project     = "mlops"
}
mlangles_mlops_dev_url_https = "https://automa.cloudangles.com"
domain = "automan.cloudangles.com"
primarydisksize = "15"
secondarydisksize = "50"

#keypair.tf
public_key = "ssh-rsa "
cidr_block = "21.0.0.0/16"
subnet_cidr = "21.0.0.0/24"
ingress_rules = [
  { port = 5000, cidr_block = "0.0.0.0/0" },
  { port = 8898, cidr_block = "0.0.0.0/0" },
  { port = 9090, cidr_block = "0.0.0.0/0" },
  { port = 8000, cidr_block = "0.0.0.0/0" },
  { port = 9001, cidr_block = "0.0.0.0/0" },
  { port = 8002, cidr_block = "0.0.0.0/0" },
  { port = 8880, cidr_block = "0.0.0.0/0" },
  { port = 9000, cidr_block = "0.0.0.0/0" },
  { port = 8086, cidr_block = "0.0.0.0/0" },
  { port = 7079, cidr_block = "0.0.0.0/0" },
  { port = 8504, cidr_block = "0.0.0.0/0" },
  { port = 3000, cidr_block = "0.0.0.0/0" },
  { port = 9092, cidr_block = "0.0.0.0/0" },
  { port = 8506, cidr_block = "0.0.0.0/0" },
  { port = 9100, cidr_block = "0.0.0.0/0" },
  { port = 8100, cidr_block = "0.0.0.0/0" },
  { port = 8101, cidr_block = "0.0.0.0/0" },
  { port = 8823, cidr_block = "0.0.0.0/0" },
  { port = 8083, cidr_block = "0.0.0.0/0" },
  { port = 8085, cidr_block = "0.0.0.0/0" },
  { port = 8885, cidr_block = "0.0.0.0/0" },
  { port = 8087, cidr_block = "0.0.0.0/0" },
  { port = 8080, cidr_block = "0.0.0.0/0" },
  { port = 22,   cidr_block = "0.0.0.0/0" },
  { port = 80,   cidr_block = "0.0.0.0/0" },
  { port = 443,   cidr_block = "0.0.0.0/0" }
]
env = "dev"
mlops_gpuec2_instance_type = "g4dn.xlarge"
mlops_gpu_rootvolume_size = "50"
mlops_gpu_secondary_volume_size = "70"
gpuami = "ami-00a4dc78b2661d64d"
ingress_gpu_rules = [
  { port = 7000, cidr_block = "0.0.0.0/0" },
  { port = 22,   cidr_block = "0.0.0.0/0" },
  { port = 80,   cidr_block = "0.0.0.0/0" },
  { port = 443,   cidr_block = "0.0.0.0/0" }
]
gpupublic_key = "ssh-rsa "

release_version = "Release-v1.0"

email = "cloud.supportteam@cloudangles.com"

#nginx_root_path = "/mnt/user-data/mlops-frontend/build"

UBUNTU_PASSWORD = "1Lk73964RZUBnZR6BNz7T74icPFfmuLeNb"